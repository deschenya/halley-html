...
const PHYSICS_REVISION='0.14-launch-lane-aware-layout';
...
function hazardsFor(stage){
 if(stage<16)return [];
 const hazards=[
  {id:'hazard-top-left',type:'spike',x:105,y:128,r:9},
  {id:'hazard-top-right',type:'spike',x:301,y:128,r:9}
 ];
 if(stage>=21){const type=stage%2?'hole':'spike';hazards.push(
  {id:'hazard-low-left',type,x:150,y:558,r:9},
  {id:'hazard-low-right',type,x:256,y:558,r:9}
 );}
 return hazards;
}
...
function safeLayout(stage){
 // The right launch chute is not part of the placement chamber.
 // The chamber center is x=204; every pair mirrors around that line while
 // the rightmost artwork remains safely left of LAUNCHER.dividerX.
 const center=204,inner=[160,248],outer=[92,316];
 const ys=[160,207,254,301,348,395,442,489];
 const points=[];
 for(let row=0;row<ys.length;row++){
  const pair=row%2===0?inner:outer;
  points.push({x:pair[0],y:ys[row]},{x:pair[1],y:ys[row]});
 }
 if(modifier(stage)===3)for(const p of points)p.x=2*center-p.x;
 return points;
}
...
function objectBounds(m,includePrice=true){
 return {left:m.x-32,right:m.x+32,top:m.y-32,bottom:m.y+(includePrice?48:32)};
}
...
function layoutAudit(stage){
 ...
 for(let i=0;i<bounds.length;i++){
   ...
   if(a.left<39||a.right>LAUNCHER.dividerX-16||a.top<124||a.bottom>550)errors.push({kind:'boundary',slot:i});
...
window.Kaskad={version:'0.14.0',snapshot:()=>copy(state),get world(){return world;},get mode(){return mode;},get ui(){return copy({...ui,focus:null});},get camera(){return renderer?renderer.camera():null;},engine:PinballWorld,config:engineConfig,calculateRate,goal,reward,skills:copy(SKILLS),types:copy(TYPES),launcher:LAUNCHER,physicsRevision:PHYSICS_REVISION,layoutAudit,featuresFor,hazardsFor,slotsFor,migrateLegacyDevices,resetFieldProgress,
